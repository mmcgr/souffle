...
Tui::Tui(std::string filename, bool live, bool /* gui */) {
    ...
    reader = std::make_shared<Reader>(filename, run);
...
Tui::Tui() {
    const std::shared_ptr<ProgramRun>& run = out.getProgramRun();
    reader = std::make_shared<Reader>(run);
    loaded = true;
    alive = true;
    updateDB();
...
std::stringstream& Tui::genJsonTop(std::stringstream& ss) {
    const std::shared_ptr<ProgramRun>& run = out.getProgramRun();
    ...
    ss << R"_({"top":[)_" << (endTime - beginTime).count() / 1000000.0 << "," << run->getTotalSize() << ","
       << run->getTotalLoadtime().count() / 1000000.0 << "," << run->getTotalSavetime().count() / 1000000.0
       << "]";
    return ss;
}

std::stringstream& Tui::genJsonRelations(std::stringstream& ss, const std::string& name, size_t maxRows) {
    const std::shared_ptr<ProgramRun>& run = out.getProgramRun();
    ...
    for (size_t i = 0; i < maxRows; ++i) {
        ...
        ss << "], ";
        std::vector<std::shared_ptr<Iteration>> iter = run->getRelation(row[5]->toString(0))->getIterations();
        ss << R"_({"tot_t": [)_";
        firstCol = true;
        for (auto& i : iter) {
            comma(firstCol);
...
std::stringstream& Tui::genJsonRules(std::stringstream& ss, const std::string& name, size_t maxRows) {
    const std::shared_ptr<ProgramRun>& run = out.getProgramRun();
...
std::stringstream& Tui::genJsonUsage(std::stringstream& ss) {
    const std::shared_ptr<ProgramRun>& run = out.getProgramRun();
...
std::stringstream& Tui::genJsonConfiguration(std::stringstream& ss) {
    auto comma = [&ss](bool& first, const std::string& delimiter = ", ") {
        if (!first) {
            ss << delimiter;
        } else {
    ...
    bool firstRow = true;
    for (auto& kvp : ProfileEventSingleton::instance().getDB().getStringMap({"program", "configuration"})) {
        comma(firstRow);
...
std::stringstream& Tui::genJsonAtoms(std::stringstream& ss) {
    const std::shared_ptr<ProgramRun>& run = out.getProgramRun();
...
void Tui::outputHtml(std::string filename) {
    std::cout << "SouffleProf\n";
...
void Tui::usageRelation(std::string id) {
...
void Tui::usageRule(std::string id) {
...
std::set<Tui::Usage> Tui::getUsageStats(size_t width) {
    ...
    for (auto& currentKey : usageStats->getKeys()) {
        ...
        cur = dynamic_cast<SizeEntry*>(usageStats->readDirectoryEntry(currentKey)->readEntry("systemtime"))
                      ->getSize();
...
void Tui::usage(uint32_t height) {
    usage({}, {}, height);
}

void Tui::usage(std::chrono::microseconds endTime, std::chrono::microseconds startTime, uint32_t height) {
...
void Tui::memoryUsage(uint32_t height) {
    memoryUsage({}, {}, height);
}

void Tui::memoryUsage(
        std::chrono::microseconds /* endTime */, std::chrono::microseconds /* startTime */, uint32_t height) {
    uint32_t width = getTermWidth() - 8;
...
void Tui::top() {
    ...
    auto* totalRulesEntry = dynamic_cast<TextEntry*>(
            ProfileEventSingleton::instance().getDB().lookupEntry({"program", "configuration", "ruleCount"}));
    ...
    std::printf("%11s%10s%10s%10s%10s%14s\n", run->getRuntime().c_str(),
            ...
            run->formatNum(0, totalRules).c_str(), run->formatNum(precision, run->getTotalSize()).c_str());
...
void Tui::rel(size_t limit, bool showLimit) {
...
void Tui::rul(size_t limit, bool showLimit) {
    ...
    for (auto& row : Tools::formatTable(ruleTable, precision)) {
        ...
        std::printf("%8s%8s%8s%8s%8s%8s %s\n", row[0].c_str(), row[1].c_str(), row[2].c_str(), row[4].c_str(),
                row[9].c_str(), row[6].c_str(), row[7].c_str());
    }
}
...
void Tui::graphByTime(std::vector<std::chrono::microseconds> list) {
...
void Tui::graphBySize(std::vector<size_t> list) {
    size_t max = 0;
    for (auto& l : list) {
        if (l > max) {
            max = l;
