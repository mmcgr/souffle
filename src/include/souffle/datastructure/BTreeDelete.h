...
protected:
    ...
    struct node : public base {
        ...
        void split(std::atomic<node*>& root, lock_type& root_lock, int idx, std::vector<node*>& locked_nodes) {
            ...
            assert((this->parent.load() == nullptr) || this->parent.load()->lock.is_write_locked());
            assert((this->parent.load() != nullptr) || root_lock.is_write_locked());
            assert(this->isLeaf() || souffle::contains(locked_nodes, this));
            assert((this->parent.load() == nullptr) || souffle::contains(locked_nodes, this->parent.load()));
#else
    ...
    private:
        ...
        void grow_parent(std::atomic<node*>& root, lock_type& root_lock, node* sibling) {
            ...
            if (this->parent.load() == nullptr) {
                assert(root.load() == this);
    ...
    public:
        ...
        bool check(Comp& comp, const node* root) const {
            ...
            // check fill-state
            if (this->numElements > maxKeys || (this->parent.load() != nullptr && this->numElements < minKeys)) {
...
public:
    ...
    btree_delete(btree_delete&& other)
            : comp(other.comp), weak_comp(other.weak_comp), root(other.root.load()), leftmost(other.leftmost) {
        other.root = nullptr;
...
public:
    ...
    size_type size() const {
        return (root.load() != nullptr) ? root.load()->countEntries() : 0;
    }
    ...
    void erase(iterator& iter) {
        ...
        iterator res(iter);
        while (true) {
            auto parent = iter.cur->parent.load();
            if (!parent) {
                // cur is root
